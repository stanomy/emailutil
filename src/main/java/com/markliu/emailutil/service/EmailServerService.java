...
public class EmailServerService {
	...
	public EmailServerInfo getConfigEmailServerInfo() {
		...
		try {
			...
			if (mailServer_POP3Host != null
					&& !("".equals(mailServer_POP3Host.trim()))) {
				emailServerInfo.setMailServerPOP3Host(mailServer_POP3Host
						.trim());
			}
			if (mailServer_SMTPHost != null
					&& !("".equals(mailServer_SMTPHost.trim()))) {
			...
			if (validate != null && !("".equals(validate.trim()))) {
				boolean isValidate = "true".equals(validate.trim()) ? true
						: false;
	...
	public Session loginEmailServer(EmailServerInfo emailServerInfo,
		...
		try {
			...
			sendMailSession = Session.getDefaultInstance(properties,
					useReadProtocol ? null : authentication);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	public boolean forwardEmail(EmailServerInfo emailServerInfo, int msgnum,
		...
		try {
			...
			try {
				...
				t.connect(emailServerInfo.getUserName(),
						emailServerInfo.getPassword());
	 ...
	 * @return
	 */
	public boolean replyEmail(EmailServerInfo emailServerInfo, int msgnum,
	 ...
	 * @return
	 */
	public boolean deleteEmailByMsgNum(EmailServerInfo emailServerInfo,
			int msgnum) {
		try {
	 ...
	 * @return
	 */
	public boolean sendEmail(Session sendMailSession,
			EmailServerInfo emailServerInfo, EmailInfo email) {
		try {
			// 根据session创建一个邮件消息
	 ...
	 * @return
	 */
	public EmailInfo getLatestOneEmailFromStore(Session sendMailSession,
			EmailServerInfo emailServerInfo) {
		EmailInfo emailInfo = null;
		Store store = null;
		try {
			store = sendMailSession.getStore("pop3");
			store.connect(emailServerInfo.getUserName(),
					emailServerInfo.getPassword());

			FetchingEmailUtil fetchingEmailUtil = new FetchingEmailUtil(
					emailServerInfo);

			emailInfo = fetchingEmailUtil.fetchingLatestEmailFromStore(store,
					true);
	...
	public List<EmailInfo> getEmailBySubjectPrefix(Session sendMailSession,
			EmailServerInfo emailServerInfo) {
	 ...
	 * @return
	 */
	public List<EmailInfo> readAllEmailInfos(Session sendMailSession,
			EmailServerInfo emailServerInfo) {
	...
	private Properties getProperties(EmailServerInfo emailServerInfo,
		...
		if (useReadProtocol) {
			...
			p.put("mail.pop3.auth", emailServerInfo.isValidate() ? "true"
					: "false");
			...
			p.put("mail.smtp.auth", emailServerInfo.isValidate() ? "true"
					: "false");
